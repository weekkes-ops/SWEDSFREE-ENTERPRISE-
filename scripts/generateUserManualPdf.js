...
export function buildUserManualDoc() {
  const doc = new jsPDF({
  ...
  const emerald900 = [6, 78, 59];
  const woodBrown = [120, 53, 15];
  const lightBg = [248, 250, 252];
  const borderColor = [226, 232, 240];

  function drawHeader(chapterTitle) {
  ...
  function addChapterHeading(num, title) {
  ...
  function addParagraph(text, extraSpacing = 4) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.8);
  ...
  function addBullet(bulletTitle, bulletDesc) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...slate700);
    const remainingWidth = contentWidth - 4 - titleWidth;
  ...
  function addCalloutBox(title, message, type = 'tip') {
    checkPageBreak(25);
  ...
  // PAGE 1: COVER PAGE
  // ==========================================
  // Deep elegant slate background accent
  doc.setFillColor(...slate900);
  ...
  // Decorative woodwork geometric accent
...
// Generate the physical file if run via node
if (process.argv[1] && process.argv[1].endsWith('generateUserManualPdf.js')) {
  try {
    const doc = buildUserManualDoc();
    const buffer = Buffer.from(doc.output('arraybuffer'));
    const targetPath = path.resolve('public', 'Swedswood_Woodwork_System_User_Manual.pdf');
    fs.writeFileSync(targetPath, buffer);
    console.log(`Successfully generated User Manual PDF at: ${targetPath} (${buffer.length} bytes, ${doc.getNumberOfPages()} pages)`);
  } catch (err) {
