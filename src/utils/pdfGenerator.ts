  ...
  if (isSwedsWood) {
    ...
    const drawSwedsHeader = (isContinuation: boolean = false) => {
      ...
      const metaData = [
        ...
        { label: "Address", val: "2 Swed Free Avenue, Sussex" },
...
export function buildReceiptPdfContent(
  ...
  doc.text("Sweds Wood Enterprise — Official Carpentry & Woodwork Clearance Receipt — 2 Swed Free Avenue, Sussex", 105, 283, { align: 'center' });
}
...
export function buildProformaInvoicePdfContent(
  ...
  const drawHeader = (isContinuation: boolean = false) => {
    ...
    doc.text("2 Swed Free Avenue, Sussex • Tel: +232 76 442590 • Email: swedswoodinfo@gmail.com", 38, 29);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(
      `Page ${p} of ${totalPages} — Sweds Wood Enterprise Official Commercial Proforma Invoice — 2 Swed Free Avenue, Sussex`,
      105,
      283,
      { align: 'center' }
    );
